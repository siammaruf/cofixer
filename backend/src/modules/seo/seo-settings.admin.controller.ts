...
import { SuccessResponseDto, UpdatedResponseDto, CreatedResponseDto } from '../../shared/dtos/response.dto';
import { SeoSettingsService } from './seo-settings.service';
import { SeoSettings } from './seo-settings.entity';
import { CreateSeoSettingsDto, UpdateSeoSettingsDto } from './dto';
...
export class SeoSettingsAdminController {
    ...
    @ApiSwagger({ resourceName: 'SEO Settings', operation: 'getAll', isArray: true, requiresAuth: true })
    ...
    @Roles(RolesEnum.ADMIN, RolesEnum.MODERATOR)
    @HttpCode(HttpStatus.OK)
    @ApiSwagger({ resourceName: 'SEO Settings', operation: 'getOne', requiresAuth: true })
    ...
    @Roles(RolesEnum.ADMIN, RolesEnum.MODERATOR)
    @HttpCode(HttpStatus.OK)
    @ApiSwagger({ resourceName: 'SEO Settings', operation: 'update', requiresAuth: true })
    async updateByRoute(
        @Param('route') route: string,
        @Body() updateDto: UpdateSeoSettingsDto,
    ): Promise<UpdatedResponseDto<SeoSettings>> {
        const existing = await this.seoSettingsService.findByRouteOrDefault(route);
        if (existing.id) {
            const updated = await this.seoSettingsService.update(existing.id, updateDto as any);
            return new UpdatedResponseDto(updated!, 'SEO settings updated successfully');
        }
        const created = await this.seoSettingsService.create({ ...updateDto, route } as any);
        return new UpdatedResponseDto(created, 'SEO settings created successfully');
    }
}
