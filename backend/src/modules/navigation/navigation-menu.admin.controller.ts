...
export class NavigationMenuAdminController {
    ...
    @ApiSwagger({ resourceName: 'Navigation', operation: 'getAll', isArray: true, requiresAuth: true })
    async findAll(): Promise<SuccessResponseDto<NavigationMenu[]>> {
        const menus = await this.navigationMenuService.findAll();
        return new SuccessResponseDto(menus, 'Navigation menus retrieved successfully');
    }

    @Put()
    @Roles(RolesEnum.ADMIN, RolesEnum.MODERATOR)
    @HttpCode(HttpStatus.OK)
    @ApiSwagger({ resourceName: 'Navigation', operation: 'update', requiresAuth: true })
    async update(@Body() updateDto: UpdateNavigationMenuDto): Promise<UpdatedResponseDto<NavigationMenu>> {
        const existing = await this.navigationMenuService.findByNameOrDefault(updateDto.name);
        if (existing.id) {
            const updated = await this.navigationMenuService.update(existing.id, {
                items: updateDto.items,
                isActive: updateDto.isActive,
            } as any);
            return new UpdatedResponseDto(updated!, 'Navigation menu updated successfully');
        }
        ...
        return new UpdatedResponseDto(created, 'Navigation menu created successfully');
    }
}
