...
export class ProjectAdminController {
    ...
    @ApiSwagger({ resourceName: 'Projects', operation: 'getAll', isArray: true, requiresAuth: true, withPagination: true })
    ...
    @Roles(RolesEnum.ADMIN, RolesEnum.MODERATOR)
    @HttpCode(HttpStatus.OK)
    @ApiSwagger({ resourceName: 'Project', operation: 'getOne', requiresAuth: true })
    async findOne(@Param('id', ParseUUIDPipe) id: string): Promise<SuccessResponseDto<Project>> {
    ...
    @ApiSwagger({ resourceName: 'Project', operation: 'create', requiresAuth: true })
    async create(@Body() createDto: CreateProjectDto): Promise<CreatedResponseDto<Project>> {
    ...
    @Roles(RolesEnum.ADMIN, RolesEnum.MODERATOR)
    @HttpCode(HttpStatus.OK)
    @ApiSwagger({ resourceName: 'Project', operation: 'update', requiresAuth: true })
    async update(@Param('id', ParseUUIDPipe) id: string, @Body() updateDto: UpdateProjectDto): Promise<UpdatedResponseDto<Project>> {
        const project = await this.projectService.update(id, updateDto as any);
    ...
    @Roles(RolesEnum.ADMIN, RolesEnum.MODERATOR)
    @HttpCode(HttpStatus.OK)
    @ApiSwagger({ resourceName: 'Project', operation: 'delete', requiresAuth: true })
    ...
    @Roles(RolesEnum.ADMIN, RolesEnum.MODERATOR)
    @HttpCode(HttpStatus.OK)
    @ApiSwagger({ resourceName: 'Project', operation: 'custom', requiresAuth: true })
    async toggleFeatured(@Param('id', ParseUUIDPipe) id: string): Promise<UpdatedResponseDto<Project>> {
        const project = await this.projectService.toggleFeatured(id);
        return new UpdatedResponseDto(project!, 'Project featured status toggled');
    }
}
